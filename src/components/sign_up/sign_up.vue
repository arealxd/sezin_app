  ...
  <div class="background">
    <div class="content">
      ...
      <div class="main-content">
        <p class="header-text">{{ $t("SIGNUP") }}</p>
        <p class="header-hint">{{ $t("UPON") }}</p>
        <input :placeholder="$t('FULLNAME')" type="text" v-model="fullName" />
        <input :placeholder="$t('EMAIL')" type="email" v-model="email" />
        <input :placeholder="$t('PASSWORD')" type="password" v-model="password" />
        <input :placeholder="$t('CONFIRMPASSWORD')" type="password" v-model="confirmPassword" />
        <p class="login-button" @click="signUp">{{ $t("SIGNUP") }}</p>
        <p
          class="header-hint"
          v-if="isShowError"
        ...
        <p class="operation-hint under-text" @click="startLogin">
          {{ $t("ALREADYMEMBER") }} <span class="sign-up-button">{{ $t("LOGIN") }}</span>
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    signUp() {
      if (this.notEmpty(this.email, this.fullName, this.password, this.confirmPassword)) {
        if (this.password === this.confirmPassword) {
          if (this.password.length > 7) {
            api.authorization
              ...
              .catch(() => {
                this.errorMessage = this.$t("EMAILALREADYREGISTERED")
                this.isShowError = true
              })
          } else {
            this.errorMessage = this.$t("8CHARACTERS")
            this.isShowError = true
          }
        } else {
